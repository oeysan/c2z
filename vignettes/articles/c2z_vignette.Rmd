---
title: "Behold c2z!"
bibliography: '`r gsub("\\\\", "/", file.path(tempdir(), file.path("c2z_temp", "references.bib")))`'
csl: '`r gsub("\\\\", "/", file.path(tempdir(), file.path("c2z_temp", "style.csl")))`'
lang: "en-US"
---
...
are doing this is to demonstrate the duplicate function in `CristinWrangler`, 
which will identify any Cristin references imported into Zotero if 
`zotero.check = TRUE`. 

The function will try to use the `zotero` argument to search the specified 
collection(s), if the argument is empty, it will search the entire Zotero 
library. We will not bother to post the remaining results, however, let's use
the `ZoteroIndex` to create an index over the collected items. 
...
# Create an index
items <- ZoteroIndex(cristin2$export)
...
gov.items <- dplyr::bind_rows(
  ...
  ZoteroGov(c("26 (2001-2002)", "31 (2014-2015)"), type = "meldst"),
  # Finds some random official Norwegian reports.
  ZoteroGov(c("2014: 4", "2018: 2"), type = "nou")
)
...
items <- ZoteroIndex(post.copy$items)
...
bibliography <- Zotero(
  ...
  format = "biblatex",
  save.data = TRUE,
  save.path = file.path(tempdir(), "c2z_temp"),
  bib.name = "references",
...
# What do we got?
sprintf(
  "We now have %s collections and %s items",
